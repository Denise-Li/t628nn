...
int main(int argc, char ** argv) {
	...
	auto host_x = std::vector<float>(size);
	auto host_y = std::vector<float>(size);
	...
	auto biasInitStatus = clblast::Copy<float>(size, device_x(), 0, 1, device_y(), 0, 1, &queue_plain, &event);
	

	// Record the execution time
	if (biasInitStatus != clblast::StatusCode::kSuccess) {
		printf("ERROR! xCOPY Status: %d\n", static_cast<int>(biasInitStatus));
		exit(-1);
	}
	...
	printf("%.3lf microseconds - Completed xCOPY.\n", time_ms);
